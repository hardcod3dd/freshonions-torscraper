...
def inject_counts():
    ...
    domain_count = cache_memoize(
        "count.domain",
        lambda: count(
            d
            for d in Domain
            if d.is_up is True
            and d.is_crap is False
            and d.is_subdomain is False
            and d.is_banned is False
        ),
    )
    day_count = cache_memoize(
        "count.new_today",
        lambda: count(
            d
            for d in Domain
            ...
            and d.is_subdomain is False
            and d.is_banned is False
            and d.created_at > event_horizon
        ),
    )
...
def json():
    ...
    domains = Domain.select(
        lambda p: p.last_alive > event_horizon and p.is_banned == False
...
def whatweb_list(name):
    ...
    domains = WebComponent.find_domains(
        name, version=version, account=account, string=string
    )
    return render_template(
        "whatweb_list.html",
        domains=domains,
...
def language_list(code):
    ...
    if len(domains) != 0:
        ...
        return render_template(
            "language_list.html", domains=domains, code=code, language=language
        )
    else:
        return (
            render_template(
                "error.html", code=404, message="No domains with language '%s'." % code
            ),
            404,
        )
...
def path_list(path):
    ...
    if len(domains) != 0:
        ...
        return (
            render_template(
                "error.html", code=404, message="Path '%s' not found." % path
            ),
            404,
        )
...
def ssh_list(id):
    ...
    if fp:
        ...
        return render_template(
            "ssh_list.html", id=id, domains=domains, fingerprint=fingerprint
        )
    else:
        return (
            render_template("error.html", code=404, message="Fingerprint not found."),
            404,
        )
...
def port_list(ports):
    ...
    for p in port_list_s:
        try:
            ...
            pass
    port_list_str = ", ".join(
        ["%s:%s" % (str(p), portscanner.get_service_name(p)) for p in port_list]
    )
    domains = select(
        d
        for d in Domain
        for op in OpenPort
        if op.domain == d and op.port in port_list and not d.is_banned
    )
    if len(domains) > 0:
        return render_template(

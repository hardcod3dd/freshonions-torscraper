...
class Domain(db.Entity):
    ...
    web_components = Set(
        "WebComponent",
        table="web_component_link",
        reverse="domains",
        column="web_component",
    )
    ...
    def domains_for_path(klass, path):
        ...
        if interesting_paths.is_php(path):
            d = left_join(
                ...
                for d in klass
                for p in d.pages
                if d.useful_404_php == True and p.code in [200, 206] and p.path == path
            )
            ...
            d = left_join(
                ...
                for d in klass
                for p in d.pages
                if d.useful_404_dir == True and p.code in [200, 206] and p.path == path
            )
        else:
            d = left_join(
                d
                for d in klass
                for p in d.pages
                if d.useful_404 == True and p.code in [200, 206] and p.path == path
            )
        return d
    ...
    def interesting_paths(self):
        ...
        if self.useful_404:
            paths += select(
                p.path
                for p in tor_db.models.page.Page
                if p.domain == self
                and p.path in interesting_paths.PATHS_OTH
                and p.code in [200, 206]
            )
        if self.useful_404_dir:
            paths += select(
                ...
                for p in tor_db.models.page.Page
                if p.domain == self
                and p.path in interesting_paths.PATHS_DIR
                and p.code in [200, 206]
            )
        if self.useful_404_php:
            paths += select(
                ...
                for p in tor_db.models.page.Page
                if p.domain == self
                and p.path in interesting_paths.PATHS_PHP
                and p.code in [200, 206]
            )
        return paths
    ...
    @db_session
    def banned(klass):
        return select(d for d in klass if d.is_banned == True).order_by(
    ...
    def before_insert(self):
        if (
            self.title.find("Site Hosted by Freedom Hosting II") != -1
            or self.title.find("Freedom Hosting II - hacked") != -1
            or self.title.find("This site is hosted by Freedom Hosting III") != -1
            or self.title.find("The Onion Farm") != -1
            or self.title.find("Site hosted by Daniel's hosting service") != -1
        ):
            self.is_crap = True
        else:
            self.is_crap = False
        ...
        if (
            not self.is_genuine
            and len(self.title) > 8
            and not self.title
            in ["Entry Point", "Login", "404 Not Found", "403 Forbidden"]
        ):
            genuine_exists = select(
                d.is_genuine
                for d in Domain
                if d.is_genuine == True and self.title == d.title
            ).first()
            if genuine_exists:
                self.is_fake = True
    ...
    def links_to(self):
        return select(
            ...
            for d in Domain
            for p in d.pages
            for link_to in p.links_to
            if link_to.domain == self
        )
    ...
    def links_from(self):
        return select(
            ...
            for d in Domain
            for p in d.pages
            for link_from in p.links_from
            if link_from.domain == self
        )

    @db_session
    def emails(self):
        return select(
            e for e in tor_db.models.email.Email for p in e.pages if p.domain == self
        )
    ...
    def bitcoin_addresses(self):
        return select(
            b
            for b in tor_db.models.bitcoin_address.BitcoinAddress
            for p in b.pages
            if p.domain == self
        ).limit(100)
    ...
    def make_genuine(klass, host):
        ...
        if domain.title.strip() != "" and not domain.title in ["Login", "Entry Point"]:
            fakes = select(
                d for d in Domain if d.is_genuine == False and d.title == title
            )
            for fake in fakes:
                fake.is_fake = True

...
def build_search_context():
    ...
    context["search_title_only"] = (
        "on"
        if (not is_elasticsearch_enabled() or request.args.get("search_title_only"))
        else None
    )
...
def count_ports(port):
    return cache_memoize(
        "count.ports:%d" % port, timeout=3600, func=lambda: OpenPort.count_open(port)
    )
...
def count_emails(email):
    return cache_memoize(
        "count.emails:%s" % email.address,
        timeout=3600,
        func=lambda: len(email.domains()),
    )
...
def count_paths(path):
    return cache_memoize(
        "count.paths:%s" % path,
        timeout=3600,
        func=lambda: len(Domain.domains_for_path(path)),
    )
...
def count_webcomponent(name, version=None, account=None, string=None):
    return cache_memoize(
        (
            "count.webcomponent:%s;%s;%s;%s"
            % (
                str(name).replace(" ", "_"),
                str(version).replace(" ", "_"),
                str(account).replace(" ", "_"),
                str(string).replace(" ", "_"),
            )
        ),
        timeout=3600,
        func=lambda: len(
            WebComponent.find_domains(
                name, version=version, string=string, account=account
            )
        ),
    )
